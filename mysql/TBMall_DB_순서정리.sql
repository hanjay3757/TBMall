...
insert into tbmall_member (member_id,member_pw,member_nick,member_gender,member_birth,member_phone,member_email,member_joindate) values('test',1234,'testtest1','m','1996-06-14','010-0000-0000','adfad@adfa.com',sysdate());
...
ALTER TABLE tbmall_stuff DROP COLUMN image_url;
ALTER TABLE tbmall_stuff 
ADD COLUMN image_url varchar(800) null;
select * from tbmall_stuff;
SELECT item_id, item_name, image_url FROM tbmall_stuff WHERE item_delete = 0;
-- 안될경우 맨마지막 숫자인 어드민번호 확인해보기.
...
VALUES 
    ('아이폰 15', 1200000, 50, 'Apple 최신 모델, 고급 스마트폰', 2),
    ('갤럭시 S24', 1100000, 30, 'Samsung 최신 모델, 성능 좋은 스마트폰', 2),
    ('에어팟 프로 2', 250000, 100, 'Apple 무선 이어폰, 고음질', 2),
    ('삼성 65인치 TV', 1500000, 20, '4K 해상도, 스마트 TV', 2);
    
ALTER TABLE tbmall_stuff ADD COLUMN image_url VARCHAR(255);
SHOW COLUMNS FROM image_url;
...
-- //하셔야 합니다 
ALTER TABLE tbmall_orders 
DROP FOREIGN KEY tbmall_orders_ibfk_1;
