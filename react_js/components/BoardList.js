...
function BoardList({ isLoggedIn, isAdmin }) {
  ...
  const loadBoards = useCallback(async () => {
    try {
      const response = await axios.get('http://192.168.0.128:8080/mvc/board/list', {
        params: { currentPage, pageSize }, // 페이징 추가
    ...
    try {
      ...
      if (window.confirm('이 글을 삭제하시겠습니까?')) {
        const response = await axios.post(
          'http://192.168.0.128:8080/mvc/board/deleteOneContent',
          { board_no: board_no },
          {
            withCredentials: true,
            headers: {
